...
const List = ({ Animes }) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <main className="BodyList h-full overflow-x-scroll">
        <ul className="flex gap-5 px-5 h-[200px]">
          {Animes.map((anime) => {
            return (
              <Link
                key={anime.id}
                to={`/${anime.id}`}
                className="AnimeList cursor-pointer min-w-[150px] max-w-[150px] rounded-lg relative "
              >
                <img
                  src={anime.image}
                  alt={anime.title}
